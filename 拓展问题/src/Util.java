...
public interface Util {
    ...
    //点P到线段AB的距离
    static double distance(Entity P, Entity A, Entity B){
        double AB = distance(A, B);
        double ABx = B.x - A.x;
        double ABy = B.y - A.y;
        double APx = P.x - A.x;
        double APy = P.y - A.y;
        double delta = (ABx * APx + ABy * APy) / (AB * AB);
    ...
    //点p到直线AB的距离
    static double pointToLine(Entity P, Entity A, Entity B){
        if(A.x == B.x){
            return Math.abs(P.x - A.x);
        }else{
            return Math.abs((A.y - B.y) / (A.x -B.x) * P.x - P.y + A.y - (A.y - B.y) / (A.x - B.x) * A.x)
                    / Math.pow((A.y - B.y) / (A.x -B.x) * (A.y - B.y) / (A.x -B.x) + 1, 0.5);
        }
    }
    //传感器是否被路径覆盖
    static boolean isCover(Sensor s, Route r){
        return distance(s, r.node1, r.node2) <= Charger.D || distance(s, r.node2, r.node3) <= Charger.D
                || distance(s, r.node3, r.node4) <= Charger.D || distance(s, r.node4, r.node1) <= Charger.D;
    }
    //计算感知质量
