...
public class TestL {
    ...
    public static void main(String[] args) throws IOException {
        ...
        //参数变化值列表
        int addNum = 100;
        int[] LList = {200, 300, 400, 500, 600, 700};
        //存放所有结果
        double[][][] res = new double[t][6][LList.length];
        long[][][] runTime = new long[t][6][LList.length];
        for(int i = 0; i < t; i++){
            //初始化网络
            Algorithm.init(120, 50, 100);
            for(int j = 0; j < LList.length; j++){
                long start = 0;
                long end = 0;
                //每次加addNum个兴趣点
                Algorithm.add(0, 0, addNum);
        ...
        for(int k = 0; k < LList.length; k++){
            ...
            //输出结果到控制台
            {
                System.out.println(type + " = " + LList[k]);
